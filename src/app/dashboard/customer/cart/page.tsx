...
export default function CustomerCart() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-32">
      ...
      {cart.length === 0 ? (
        <div className="px-6 py-20 flex flex-col items-center gap-6 text-center">
          ...
          <div className="space-y-1">
            <p className="font-semibold text-xl text-slate-900">Your cart is empty</p>
            <p className="text-sm text-slate-500 font-medium">Add some fresh groceries to get started.</p>
          </div>
          <Button onClick={() => router.push('/dashboard/customer')} className="rounded-2xl h-14 px-8 font-semibold bg-primary">Start Shopping</Button>
        </div>
      ) : (
        <div className="px-6 space-y-4 mt-2">
          <div className="space-y-3">
            {cart.map((item) => (
              <div key={item.productId} className="bg-white rounded-[2rem] p-4 flex gap-4 shadow-sm border border-slate-100/50 relative">
          ...
          <div className="bg-white rounded-[2rem] p-4 shadow-sm border border-slate-100/50 flex items-center gap-3">
            <TicketPercent className="h-5 w-5 text-slate-400" />
            <input 
              placeholder="Apply a promo code" 
              className="flex-1 border-none bg-transparent h-auto p-0 focus:ring-0 font-medium text-sm outline-none"
            />
            <Button size="sm" className="rounded-xl px-6 bg-primary font-semibold">Apply</Button>
          </div>

          <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100/50 space-y-5">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="space-y-2">
                <Label className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 ml-1">Full Name</Label>
                <Input 
                  placeholder="Your Name" 
                  className="bg-slate-50 border-none rounded-2xl h-12 font-medium px-4 focus-visible:ring-primary shadow-inner"
                  value={customerName}
                  onChange={(e) => setCustomerName(e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <Label className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 ml-1">Contact Number</Label>
                <Input 
          ...
          <div className="bg-slate-900 rounded-[2rem] p-5 text-white flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-medium">Scan & Pay on Delivery</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100/50 space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-bold text-slate-900">₹{cartTotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm font-medium text-slate-400">Delivery</span>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-xl font-bold text-primary">₹{(cartTotal + deliveryFee).toFixed(2)}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-lg border-t border-slate-100 flex justify-center z-20">
          <Button 
            className="w-full h-16 rounded-[2rem] bg-primary font-bold text-lg shadow-xl shadow-primary/20" 
      ...
      <AlertDialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <AlertDialogContent className="rounded-[2.5rem] border-none p-8">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="font-bold text-2xl text-center">Confirm Order?</AlertDialogTitle>
            <AlertDialogDescription className="font-medium text-center text-slate-500 pt-2 leading-relaxed">
              Your fresh groceries will be delivered to <span className="text-slate-900 font-semibold">"{deliveryAddress}"</span> in 10-15 minutes.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-row gap-3 mt-8">
            <AlertDialogCancel className="flex-1 rounded-2xl h-14 border-none bg-slate-100 font-semibold text-slate-900">Cancel</AlertDialogCancel>
            <AlertDialogAction className="flex-1 rounded-2xl bg-primary h-14 font-semibold shadow-lg shadow-primary/20" onClick={handlePlaceOrder}>Confirm</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
